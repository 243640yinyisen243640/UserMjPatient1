 ...
 */
public class HealthyDietChildThreeFragment extends BaseEventBusFragment {
    ...
    protected void receiveEvent(EventMessage event) {
        ...
        switch (event.getCode()) {
            case ConstantParam.GREEN_SEARCH:
                String currentPositionSearch = event.getMsg();
                String keyWordSearch = (String) event.getData();
                if (currentFragmentPosition.equals(currentPositionSearch)) {
                    getSearchData(keyWordSearch);
                }
                break;
            case ConstantParam.GREEN_REFRESH:
                getLvData();
                break;
        }
    }
