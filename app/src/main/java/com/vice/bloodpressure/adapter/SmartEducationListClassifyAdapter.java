...
public class SmartEducationListClassifyAdapter extends BaseQuickAdapter<SmartEducationBean, BaseViewHolder> {

    private Context context;

    public SmartEducationListClassifyAdapter(@Nullable List<SmartEducationBean> data, Context context) {
        super(R.layout.item_smart_education, data);
        this.context = context;
    }

    @Override
    protected void convert(@NonNull BaseViewHolder viewHolder, SmartEducationBean listBean) {
        ...
        //列表
        if (listBean.getArtlist() != null && listBean.getArtlist().size() > 0) {
            MyListView lvLearnList = viewHolder.getView(R.id.lv_learn_list);
            SmartEducationClassifyListChildAdapter adapter = new SmartEducationClassifyListChildAdapter(context, R.layout.item_smart_education_child, listBean.getArtlist(), learning);
